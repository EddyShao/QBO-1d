...
def sample_sf_cw_complex(n, sfe, sfv, cwe, cwv, corr, seed):
    sf, cw = sample_sf_cw(n, sfe, sfv, cwe, cwv, corr, seed)
    
    # k_sf = torch.linspace(1, 0.1, n) # With this coefficient, the variance is minimized to 0.1
    # b_sf = torch.linspace(0, 1e-3, n) 
    

    # b_cw = torch.linspace(0, 10, n)
    
    # return k_sf * sf + b_sf, cw + b_cw
    # # return sf, cw
    ...
    b_sf = torch.tensor([(i/n) * b_sf_max * np.sin((i/n) * k * np.pi) for i in range(n)])
    b_cw = torch.tensor([(i/n) * b_cw_max * np.cos((i/n) * k * np.pi) for i in range(n)])
    
    return sf + b_sf, cw + b_cw
    
    

class ComplexWaveSpectrum(torch.nn.Module):
    ...
    def __init__(self, solver,
        ...
        # surface fluxes and spectral widths
        self.sf, self.cw = sample_sf_cw_complex(n=self._nsteps,
