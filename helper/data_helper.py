...
def data_loader(state=1):
    ...
    if isinstance(state, int):
        model = WaveSpectrum(solver, 0, *para_mat[state])
    elif isinstance(state, dict):
        model = WaveSpectrum(solver, **state)
    elif isinstance(state, list):
        model = WaveSpectrum(solver, 0, *state)
...
def control_data_loader(file_path='/scratch/zs1542/QBO-1d/control.nc'):
    ds = xr.open_dataset(file_path)

    return ds.u.values, ds.S.values, ds.sf_samp.values, ds.cw_samp.values



def annual_w_data_loader(state=1):
    """Data loader(with annual cycle of w, the motivation is to create complex data)
    ...
    time = torch.arange(0, t_max+dt, dt)
    w = 3e-4 + 2e-4 * torch.sin(2 * torch.pi / 360 / 86400 * time)
...
def control_annual_w_data_loader(file_path='/scratch/zs1542/QBO-1d/annual_w_control.nc'):
    ds = xr.open_dataset(file_path)
...
def control_dataset(u, s, sf, cw, K=1, test_size=0.8):
    ...
    U = np.hstack([u[:, 1:-1], sf.reshape(-1, 1), cw.reshape(-1, 1)])
    S = s
