...
def training_1d(U_train, U_test, s_train, s_test, epsilon, C):
    report_1d = {}
    for level in [1, 35, 70]:
        ...
        with joblib.parallel_backend(backend='threading', n_jobs=16):
            svr = SVR(kernel='rbf', gamma='auto', epsilon=epsilon, C=C)
...
def training_multidim(U_train, U_test, s_train, s_test, epsilon, C):
    # Multiregression
    with joblib.parallel_backend(backend='threading', n_jobs=16):
        svr_1d = SVR(kernel = 'rbf', gamma='auto', epsilon=epsilon, C=C)
        mr = MultiOutputRegressor(svr_1d)
...
if __name__ == '__main__':
    ...
    report = {
        ...
        'C': C,
        'report_1d': report_1d,
        'report_multidim': report_multidim,
        'qbo_paras': qbo_paras,
        'duration': duration
    }
